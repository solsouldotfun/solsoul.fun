...
import { listClaimedSoulNfts, type ClaimedSoulNftPage } from "sdk";
...
export default function PublicSoulsGalleryPage() {
  return (
    <Suspense fallback={null}>
      <PublicSoulsGalleryClient />
    </Suspense>
  );
}

function PublicSoulsGalleryClient() {
  ...
  const galleryHref = "/souls";
  const cacheKey = useMemo(() => buildClaimedSoulGalleryCacheKey("public", pageNumber), [pageNumber]);
  ...
  }, [cacheKey, connection, pageNumber, t]);
        ...
        <>
          <div className="grid min-w-0 gap-3 sm:grid-cols-2 lg:grid-cols-3 2xl:grid-cols-4">
            {galleryState.items.map((item) => (
              <SoulGalleryCard item={item} key={item.claim} scope="publicGallery" />
            ))}
          </div>
